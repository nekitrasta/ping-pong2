...
def send_right():
    ...
    msg_l = c.coords(BALL)
    msg = str(msg_l[0]) + ',' + str(msg_l[1]) + ',' + str(msg_l[2]) + ',' + str(msg_l[3]) + ',' + str(
        BALL_X_CHANGE) + ',' + str(BALL_Y_SPEED)
    BALL_X_SPEED = 0
    BALL_Y_SPEED = 0
    msg = msg.encode('UTF-8')
...
c = Canvas(root, width=WIDTH, height=HEIGHT, background="#800080")
c.pack()

BALL = c.create_oval(WIDTH / 2 - BALL_RADIUS / 2,
                     HEIGHT / 2 - BALL_RADIUS / 2,
                     WIDTH / 2 + BALL_RADIUS / 2,
                     HEIGHT / 2 + BALL_RADIUS / 2, fill="orange")

LEFT_PAD = c.create_line(LEFT_PAD_W / 2, 0, LEFT_PAD_W / 2, LEFT_PAD_H, width=LEFT_PAD_W, fill="black")

RIGHT_PAD = c.create_line(WIDTH - RIGHT_PAD_W / 2, 0, WIDTH - RIGHT_PAD_W / 2,
                          RIGHT_PAD_H, width=RIGHT_PAD_W, fill="black")
